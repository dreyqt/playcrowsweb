...
function PublicHeader({ server }: { server: PlayCrowsServer }) {
  const { language } = useI18n(); const ui = UI[language]
  const serverTone = server === 'v2' ? 'border-[#a78bfa]/45 bg-[#8b5cf6]/10 text-[#c4b5fd]' : 'border-[#c9aa68]/45 bg-[#c9aa68]/10 text-[#c9aa68]'
  return <header className="sticky top-0 z-50 border-b border-[#171a20] bg-[#0a0b0d]"><div className="mx-auto flex max-w-5xl flex-wrap items-center justify-between gap-3 px-4 py-4"><a href={`/events?server=${server}`} className="flex min-w-0 items-center gap-3 no-underline"><img src={CrowLogo} alt="PlayCrows logo" className="h-10 w-10 rounded-full object-cover" /><div><div className="text-base font-bold leading-tight text-[#eee9df]">PLAYCROWS</div><div className="text-[10px] uppercase tracking-[0.18em] text-[#77746e]">{ui.eventCenter}</div></div></a><nav className="flex flex-wrap items-center justify-end gap-2 text-xs font-bold"><div className="flex rounded-lg border border-[#292d34] bg-[#0f1115] p-1"><a href="/events?server=v1" className={`rounded-md px-2.5 py-1.5 no-underline ${server==='v1'?'bg-[#c9aa68]/15 text-[#c9aa68]':'text-[#77746e] hover:text-[#aaa49a]'}`}>V1</a><a href="/events?server=v2" className={`rounded-md px-2.5 py-1.5 no-underline ${server==='v2'?'bg-[#8b5cf6]/15 text-[#c4b5fd]':'text-[#77746e] hover:text-[#aaa49a]'}`}>V2</a></div><span className={`rounded-lg border px-2.5 py-2 text-[10px] font-black uppercase tracking-wider ${serverTone}`}>PlayCrows {server.toUpperCase()}</span><LanguageSelector /><a href="/" className="rounded-lg border border-[#292d34] px-3 py-2 text-[#aaa49a] no-underline hover:border-[#c9aa68] hover:text-[#c9aa68]">{ui.webShop}</a></nav></div></header>
}
...
function EventList({ server }: { server: PlayCrowsServer }) {
  const { language } = useI18n(); const ui = UI[language]
  const [events, setEvents] = useState<PlayCrowsEvent[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState('')
  useEffect(() => { setLoading(true); setError(''); void (async () => { const { data, error: loadError } = await supabase.from('events').select('*').eq('server',server).in('status', ['active','ended']).not('published_at','is',null).order('published_at',{ ascending:false }); if (loadError) setError(loadError.message); else setEvents((data ?? []) as PlayCrowsEvent[]); setLoading(false) })() }, [server])
  const accent = server === 'v2' ? 'text-[#c4b5fd]' : 'text-[#c9aa68]'
  return <div className="min-h-screen bg-[#0a0b0d] text-[#eee9df]"><PublicHeader server={server} /><main className="mx-auto max-w-5xl px-4 py-10"><section className={`mb-9 overflow-hidden rounded-2xl border bg-[#111318] p-6 sm:p-8 ${server==='v2'?'border-[#a78bfa]/25':'border-[#c9aa68]/25'}`}><div className={`text-[10px] font-bold uppercase tracking-[0.2em] ${accent}`}>PlayCrows {server.toUpperCase()} Community</div><h1 className="mt-3 text-3xl font-extrabold tracking-tight sm:text-4xl">{ui.eventCenter}</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-[#8f8b84]">{ui.intro}</p></section>{loading && <EmptyState message={ui.loading} />}{error && <EmptyState message={`${ui.unableLoad}: ${error}`} />}{!loading && !error && events.length === 0 && <EmptyState message={ui.noEvents} />}<div className="grid gap-5 md:grid-cols-2">{events.map(raw => { const event = localizeEvent(raw, language); return <a key={event.id} href={`/events/${encodeURIComponent(event.slug)}?server=${server}`} className={`group rounded-2xl border bg-[#111318] p-6 text-inherit no-underline transition hover:-translate-y-0.5 ${server==='v2'?'border-[#292d34] hover:border-[#a78bfa]/50':'border-[#292d34] hover:border-[#c9aa68]/50'}`}><div className="flex items-start justify-between gap-4"><div><div className={`text-xs font-bold uppercase tracking-[0.15em] ${accent}`}>Event #{event.event_number}</div><h2 className={`mt-2 text-xl font-bold ${server==='v2'?'group-hover:text-[#c4b5fd]':'group-hover:text-[#e1c88d]'}`}>{event.title}</h2></div><span className={`rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${statusBadge(event.status)}`}>{event.status === 'active' ? ui.active : ui.ended}</span></div><p className="mt-4 min-h-12 text-sm leading-6 text-[#8f8b84]">{event.short_description || event.description || ''}</p><div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 border-t border-[#292d34] pt-4 text-xs text-[#77746e]">{dateLabel(event.starts_at) && <span>{ui.starts}: {dateLabel(event.starts_at)}</span>}{dateLabel(event.ends_at) && <span>{ui.ends}: {dateLabel(event.ends_at)}</span>}</div></a> })}</div></main></div>
}
...
function EventDetail({slug,server}:{slug:string;server:PlayCrowsServer}) {
  const {language}=useI18n();const ui=UI[language];const [rawEvent,setRawEvent]=useState<PlayCrowsEvent|null>(null);const [loading,setLoading]=useState(true);const [error,setError]=useState('');const [tab,setTab]=useState<'details'|'results'|'status'>('details');useEffect(()=>{setLoading(true);setError('');void(async()=>{const{data,error:loadError}=await supabase.from('events').select('*').eq('server',server).eq('slug',slug).in('status',['active','ended']).not('published_at','is',null).maybeSingle();if(loadError)setError(loadError.message);else setRawEvent(data as PlayCrowsEvent|null);setLoading(false)})()},[slug,server]);const event=rawEvent?localizeEvent(rawEvent,language):null;const canSubmit=useMemo(()=>{if(!event||event.status!=='active')return false;const now=Date.now();if(event.starts_at&&new Date(event.starts_at).getTime()>now)return false;if(event.ends_at&&new Date(event.ends_at).getTime()<now)return false;return true},[event]);const accent=server==='v2'?'text-[#c4b5fd]':'text-[#c9aa68]';return <div className="min-h-screen bg-[#0a0b0d] text-[#eee9df]"><PublicHeader server={server}/><main className="mx-auto max-w-5xl px-4 py-8 sm:py-10"><a href={`/events?server=${server}`} className={`text-xs font-bold no-underline ${accent}`}>← {ui.allEvents}</a>{loading&&<div className="mt-6"><EmptyState message={ui.loading}/></div>}{error&&<div className="mt-6"><EmptyState message={`${ui.unableLoad}: ${error}`}/></div>}{!loading&&!event&&<div className="mt-6"><EmptyState message={ui.noEvents}/></div>}{event&&<><section className={`mt-5 rounded-2xl border bg-[#111318] p-6 sm:p-8 ${server==='v2'?'border-[#a78bfa]/25':'border-[#c9aa68]/25'}`}><div className="flex flex-wrap items-start justify-between gap-4"><div><div className={`text-xs font-bold uppercase tracking-[0.18em] ${accent}`}>PlayCrows {server.toUpperCase()} · Event #{event.event_number}</div><h1 className="mt-2 text-3xl font-extrabold">{event.title}</h1></div><span className={`rounded-full border px-3 py-1 text-[10px] font-bold uppercase ${statusBadge(event.status)}`}>{event.status==='active'?ui.active:ui.ended}</span></div>{event.description&&<p className="mt-5 max-w-3xl whitespace-pre-line text-sm leading-7 text-[#aaa49a]"><RichText text={event.description}/></p>}{(event.action_links??[]).length>0&&<EventActionButtons links={event.action_links??[]}/>}<div className="mt-5 flex flex-wrap gap-4 text-xs text-[#77746e]">{dateLabel(event.starts_at)&&<span>{ui.starts}: {dateLabel(event.starts_at)}</span>}{dateLabel(event.ends_at)&&<span>{ui.ends}: {dateLabel(event.ends_at)}</span>}</div></section><div className="mt-6 flex gap-2 rounded-xl border border-[#292d34] bg-[#0f1115] p-1"><button onClick={()=>setTab('details')} className={`flex-1 rounded-lg px-3 py-2 text-xs font-bold ${tab==='details'?(server==='v2'?'bg-[#8b5cf6]/12 text-[#c4b5fd]':'bg-[#c9aa68]/12 text-[#c9aa68]'):'text-[#77746e]'}`}>{ui.mechanicsClaim}</button><button onClick={()=>setTab('results')} className={`flex-1 rounded-lg px-3 py-2 text-xs font-bold ${tab==='results'?(server==='v2'?'bg-[#8b5cf6]/12 text-[#c4b5fd]':'bg-[#c9aa68]/12 text-[#c9aa68]'):'text-[#77746e]'}`}>{ui.results}</button><button onClick={()=>setTab('status')} className={`flex-1 rounded-lg px-3 py-2 text-xs font-bold ${tab==='status'?(server==='v2'?'bg-[#8b5cf6]/12 text-[#c4b5fd]':'bg-[#c9aa68]/12 text-[#c9aa68]'):'text-[#77746e]'}`}>{ui.checkClaim}</button></div>{tab==='details'?<div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.92fr]"><div className="space-y-6"><section className="rounded-2xl border border-[#292d34] bg-[#111318] p-6"><h2 className="text-lg font-bold">📌 {ui.mechanics}</h2><ol className="mt-4 space-y-3 text-sm leading-6 text-[#aaa49a]">{(event.mechanics??[]).map((item,i)=><li key={i} className="flex gap-3"><span className={`font-bold ${accent}`}>{i+1}.</span><span><RichText text={item}/></span></li>)}</ol></section><section className="rounded-2xl border border-[#292d34] bg-[#111318] p-6"><h2 className="text-lg font-bold">🎁 {ui.rewards}</h2><div className="mt-4 space-y-2 text-sm text-[#aaa49a]">{(event.rewards??[]).map((item,i)=><div key={i}>◆ <RichText text={item}/></div>)}</div></section></div><div>{canSubmit?<SubmissionForm event={event}/>:<div className="rounded-2xl border border-[#292d34] bg-[#111318] p-6 text-center"><div className="text-2xl">🔒</div><h3 className="mt-3 font-bold">{ui.submissionsClosed}</h3><p className="mt-2 text-sm text-[#77746e]">{ui.closedDesc}</p></div>}</div></div>:tab==='results'?<div className="mt-6"><Results eventId={event.id}/></div>:<div className="mt-6"><ClaimStatusLookup/></div>}</>}</main></div>
}
function EventCenterInner(){const path=window.location.pathname.replace(/\/+$/,'')||'/events';const match=path.match(/^\/events\/([^/]+)$/);const params=new URLSearchParams(window.location.search);const server:PlayCrowsServer=params.get('server')==='v2'?'v2':'v1';return match?<EventDetail slug={decodeURIComponent(match[1])} server={server}/>:<EventList server={server}/>}
