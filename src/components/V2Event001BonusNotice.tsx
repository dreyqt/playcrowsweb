...
export function V2Event001BonusNotice() {
  const { t } = useI18n()
    ...
    >
      ...
      {expanded && (
        <div id={contentId} className="border-t border-[#8b5cf6]/25 px-4 pb-4 pt-3 sm:px-5 sm:pb-5">
          ...
          <div className="mt-3 rounded-lg border border-[#8b5cf6]/20 bg-[#111318] p-4">
            <p className="text-xs font-bold uppercase tracking-widest text-[#a78bfa]">{t('v2EventBonusBundle')}</p>
            <ul className="mt-3 grid gap-2 text-sm text-[#d7d2ca] sm:grid-cols-2">
              {EVENT001_REWARDS.map(reward => (
